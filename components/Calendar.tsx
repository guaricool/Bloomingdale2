    ...
    <div className="paper-card overflow-hidden">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
        <button
          ...
          className="flex h-7 w-7 items-center justify-center rounded-full text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-900"
        >
          ‹
        </button>
      ...
      {/* Weekday header */}
      <div className="grid grid-cols-7 border-b border-slate-200 bg-slate-100/40 text-center">
        {WEEKDAYS.map((w) => (
          <div
            key={w}
            className="py-1.5 font-sans text-[0.65rem] font-semibold uppercase tracking-wider text-slate-500"
          >
            {w}
          </div>
        ))}
      ...
      <div className="grid grid-cols-7">
        {grid.map((cell, idx) => {
          if (cell.day === null) {
            return <div key={idx} className="h-9 border-b border-slate-200/60" />;
          }
          ...
          return (
            <button
              ...
              className={clsx(
                "relative h-9 border-b border-slate-200/60 font-sans text-xs transition-all duration-200",
                "hover:bg-blue-50 hover:scale-110 hover:z-10 hover:shadow-soft focus-visible:bg-blue-50",
                isToday && "ring-1 ring-inset ring-blue-400 cell-today",
                has && "font-semibold text-slate-900",
                !has && "text-slate-500",
              )}
          ...
          >
            <div className="flex items-start justify-between border-b border-slate-200 px-5 py-4">
              <div>
                ...
                <h3 className="mt-1 font-display text-xl font-medium text-slate-900">
                  {openEvents.length === 0
                    ? "Sin eventos"
                    : openEvents.length === 1
                      ? "1 evento"
            ...
            <div className="px-5 py-4">
              {openEvents.length === 0 ? (
                <div className="flex flex-col items-center py-6 text-center">
                  <div className="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-blue-50 text-blue-600" aria-hidden>
                    <IconCalendar />
                  </div>
                ...
                <ul className="space-y-3">
                  {openEvents.map((ev) => (
                    <li
                      key={ev.id}
                      className="rounded-card border border-slate-200 bg-slate-50/60 p-3"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <p className="font-display text-base font-medium text-slate-900">
                          {ev.title}
                        </p>
                        <span className="shrink-0 rounded-pill border border-blue-200 bg-blue-50 px-2 py-0.5 font-sans text-[0.65rem] font-semibold uppercase tracking-wider text-blue-700">
                          {ev.type}
                        </span>
                      </div>
                      {ev.description ? (
            ...
            <div className="flex justify-end border-t border-slate-200 px-5 py-3">
              <button
                type="button"
                onClick={() => setOpenDay(null)}
                className="rounded-pill border border-slate-300 bg-white px-4 py-1.5 font-sans text-xs font-medium text-slate-700 transition-colors hover:bg-slate-50"
              >
                Cerrar
              </button>
            </div>
